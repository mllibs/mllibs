...
# #     define encoder
#     encoder = CountVectorizer(tokenizer=punktokeniser)
...
def tfidf(tokens:list,vectoriser=None):
    ...
    if(vectoriser == None):
        vectoriser = TfidfVectorizer(tokenizer=lambda x: punktokeniser(x),token_pattern=None)
...
def dicttransformer(tokens:list,vectoriser=None):
    ...
    token_features = extract_token_features_small(tokens)
